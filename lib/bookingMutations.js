...
import { displayServiceName } from "./serviceGroups.js";
...
const FocusAreaSchema = z.array(z.enum(FOCUS_AREA_IDS)).optional().default([]);
const AddOnSchema = z.array(z.enum(ADD_ON_IDS)).optional().default([]);
...
function formatFocusAreas(focusAreas) {
  if (!Array.isArray(focusAreas) || focusAreas.length === 0) return "None";
  return focusAreas.map((id) => FOCUS_AREA_LABELS[id] || id).join(", ");
}

function formatAddOns(addOns) {
  if (!Array.isArray(addOns) || addOns.length === 0) return "None";
  return addOns.map((id) => ADD_ON_LABELS[id] || id).join(", ");
}

export { formatFocusAreas, formatAddOns };
...
export async function sendAdminNewBookingEmail(booking) {
  ...
  const serviceName = displayServiceName(booking.service?.name) || "N/A";
...
Notes: ${notes}`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111;">
      ...
      <p><strong>Service:</strong> ${escapeHtml(serviceLine)}</p>
      <p><strong>Focus areas:</strong> ${escapeHtml(focusAreas)}</p>
...
export async function sendClientBookingApprovedEmail(booking) {
  ...
  const serviceName = displayServiceName(booking.service?.name) || "your session";
  const durationMin = booking.service?.durationMin;
  const serviceLine = durationMin ? `${serviceName} (${durationMin} min)` : serviceName;
  ...
  const focusAreas = formatFocusAreas(booking.focusAreas);
  const addOns = formatAddOns(booking.addOns);
  const hasFocusAreas = focusAreas !== "None";
  const hasAddOns = addOns !== "None";
  const subject = "Booking Created and Approved";

  const therapistLine = therapistName ? `Therapist: ${therapistName}\n` : "";
  const focusLine = hasFocusAreas ? `Focus areas: ${focusAreas}\n` : "";
  const addOnLine = hasAddOns ? `Add-ons: ${addOns}\n` : "";
...
Date & time: ${bookingTime}
Service: ${serviceLine}
${focusLine}${addOnLine}${therapistLine}${notesLine}
...
Maron's Muscle Studio`;
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.5; color: #111;">
      ...
      <p><strong>Date &amp; time:</strong> ${escapeHtml(bookingTime)}</p>
      <p><strong>Service:</strong> ${escapeHtml(serviceLine)}</p>
      ${hasFocusAreas ? `<p><strong>Focus areas:</strong> ${escapeHtml(focusAreas)}</p>` : ""}
      ${hasAddOns ? `<p><strong>Add-ons:</strong> ${escapeHtml(addOns)}</p>` : ""}
      ${
        therapistName
...
Date & time: ${bookingTime}
Service: ${serviceLine}
${focusLine}${addOnLine}Preferred therapist: ${therapistPreference}
${notesLine}
Maron's Muscle Studio`;
...
export async function buildBookingWriteData(
  ...
  const focusAreas = Array.isArray(body.focusAreas)
    ? Array.from(new Set(body.focusAreas.filter((id) => FOCUS_AREA_IDS.includes(id))))
    : [];
  const addOns = Array.isArray(body.addOns)
    ? Array.from(new Set(body.addOns.filter((id) => ADD_ON_IDS.includes(id))))
    : [];
