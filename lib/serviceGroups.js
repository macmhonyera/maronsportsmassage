...
export const SERVICE_GROUP_CONFIG = [
  ...
  },
  {
    ...
    // The shortest session only covers half the body — flagged on the duration step
    // so clients know what they're booking.
    durationNotes: { 30: "Half body" },
    styles: [
      {
        id: "structural",
        name: "Structural",
...
// Extra qualifier for a given session length (e.g. "Half body" for the 30 min
// Full Body Massage). Returns "" when the duration needs no clarification.
export function getDurationNote(serviceName, durationMin) {
  const group = findGroupForServiceName(serviceName);
  return group?.durationNotes?.[durationMin] || "";
}
